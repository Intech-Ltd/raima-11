...
static int32_t configure_slave(const char *, int32_t, uint16_t, TFS_HANDLE *, REP_HANDLE *);
...
static void EXTERNAL_FCN repSlave_dberr( int32_t, const char *, void *, DB_TASK *);
    ...
    const char *const *argv)
{
    ...
    /* Start slave threads */
    DisplayMessageNow(FALSE, FALSE, "Starting slave TFS and dbrep threads\n");
    rdm_status = configure_slave(dbrep_opts.slave_docroot, dbrep_opts.slave_port, dbrep_opts.verbose, &hTFS, &hREP);
    ...
    if(rdm_status != S_OKAY)
    {
        DisplayErrorNow(FALSE, FALSE, "Unable to connect to master for replication (%s - %s)\n", d_errorname(rdm_status), d_errorstr(rdm_status));
    }
    ...
    while (!quit)
    {
        ...
        DisplayMessage(FALSE, FALSE, "   1 - Display data in slave databases\n");
        ...
        case 'Q':
            ...
            DisplayMessageNow(FALSE, FALSE, "Cleaning up slave threads\n");
            cleanup_slave(hTFS, hREP);
            quit = 1;
            break;
        ...
        case '1':
            ...
            DisplayMessageNow(FALSE, FALSE, "Reading the data from the slave database\n");
            read_slave_data(dbrep_opts.master_host, dbrep_opts.master_port);
            break; 

        default:
            DisplayMessageNow(FALSE, FALSE, "*** incorrect command -- re-enter\n");
            break;
        }
    }
    ...
    REP_HANDLE *hREP)
{
    ...
    /* Initialize the internal slave TFS, note that there are other ways
    ...
    */
    ...
    if(rdm_status == S_OKAY)
    {
        ...
        if(rdm_status != S_OKAY)
        {
            DisplayErrorNow(FALSE, FALSE, "Error starting the slave TFS threads (%s - %s)\n", d_errorname(rdm_status), d_errorstr(rdm_status));
            return rdm_status;
        }
    }
    else
    {
        DisplayErrorNow(FALSE, FALSE, "Error initializing the slave TFS threads (%s - %s)\n", d_errorname(rdm_status), d_errorstr(rdm_status));

        return rdm_status;
    }

    /* Initialize the internal slave replication server, there are other ways to
    ...
    */
    ...
    if(rdm_status == S_OKAY)
    {
        ...
        if(rdm_status != S_OKAY)
        {
            DisplayErrorNow(FALSE, FALSE, "Error starting the slave dbrep threads (%s - %s)\n", d_errorname(rdm_status), d_errorstr(rdm_status));
            return rdm_status;
        }
    }
    else
    {
        DisplayErrorNow(FALSE, FALSE, "Error initializing the slave dbrep threads (%s - %s)\n", d_errorname(rdm_status), d_errorstr(rdm_status));
        return rdm_status;
    }
    return S_OKAY;
}
    ...
    int32_t     master_port)
{
    ...
    {
        if((rdm_status = d_set_dberr_ex(repSlave_dberr, (void*)NULL, task)) != S_OKAY)
        {
            DisplayErrorNow(FALSE, FALSE, "Error %d setting error handler (%s - %s)\n", d_errorname(rdm_status), d_errorstr(rdm_status));
            d_closetask(task);
            return S_NOMEMORY;
        }
        ...
        if(!db)
        {
            DisplayErrorNow(FALSE, FALSE, "Error out of memory (%s - %s)\n", d_errorname(rdm_status), d_errorstr(rdm_status));
            d_closetask(task);
            return S_NOMEMORY;
        }
        ...
        if ((rdm_status = d_open(db, "r", task)) != S_OKAY)
        {
            ...
            if(rdm_status == S_NOTIMPLEMENTED)
            {
                ...
                if ((rdm_status = d_open(db, "r", task)) != S_OKAY)
                {
                    DisplayErrorNow(FALSE, FALSE, "Error opening databases (%s - %s)\n", d_errorname(rdm_status), d_errorstr(rdm_status));
                    d_closetask(task);
                    return S_NOMEMORY;
                }
                num_dbs = 2;
            ...
            if(rdm_status != S_OKAY)
            {
                DisplayErrorNow(FALSE, FALSE, "Error opening databases as a union (%s - %s)\n", d_errorname(rdm_status), d_errorstr(rdm_status));
                psp_freeMemory(db, TAG0);
                d_closetask(task);
                return rdm_status;
            }
        ...
        { 
            ...
            {
                DisplayErrorNow(FALSE, FALSE, "Error reading unit record (%s - %s)\n", d_errorname(rdm_status), d_errorstr(rdm_status));
            }
            else
            {
                rdm_status = S_OKAY;
    ...
    REPL_EXAMPLE_OPTS *repl_opts)
{
    ...
    cmdline_opt opts[] = {
        ...
        { ARG_OPT,         ":v",                   NULL,           "Display verbose server messages" },
    ...
    optstat = b_cmd_initopt(RDM_EXAMPLE_DESC("Replication Slave"), argc,
...
static void EXTERNAL_FCN repSlave_dberr(
    int32_t     errnum, 
    const char *msg,
    void       *data,
    DB_TASK    *task)
