...
#include "rdm.h"
#include "base.h"
#include "initTFS_db.h"
#include "initTFS_db_dbd.h"
...
/* Main function to demonstrate initializing a TFS_TYPE_TFS tfs */
int32_t EXTERNAL_FCN initTFS_Tutorial_main(
    int32_t            argc,
...
/* This function will initialize an internal TFS, open a task, and open the 
 *   tutorial database
 */
...
{
    int32_t     rdm_status = S_OKAY;
    const char *dbname = "initTFS_db";
    TFS_PARAMS  params;

    memset(&params, 0, sizeof(params));

    params.port    = ctx->port;
    ...
    /* Initialize the TFS to use the TFS implementation 
    ...
    /* When using the TFS implementation you can start the RPC threads so
     *    the TFS will listen for connections from other applications
     *  
     *    We want to use the threaded version otherwise the d_tfsrun API
     *    will not return.
     */
    rdm_status = d_tfsrun(ctx->hTFS, TRUE, &ctx->tfs_done);
    if(rdm_status != S_OKAY)
    {
        DisplayErrorNow(FALSE, FALSE, 
                "Error start TFS listening and connection threads(%s - %s)\n", 
                d_errorname(rdm_status), d_errorstr(rdm_status));
        return rdm_status;
    }
    DisplayMessageNow(FALSE, FALSE, "Successfully started the TFS listening and connection threads\n");
    ...
    rdm_status = d_open_ptr(dbname, openmode, &initTFS_db_dbd, sizeof(initTFS_db_dbd), ctx->task);
    if(rdm_status != S_OKAY)
    {
    ...
    TFS_INIT_CTX *ctx)
{
    ...
    /* Stop the TFS listening and connection threads */
    d_tfsstop(ctx->hTFS);
    ...
    TFS_INIT_CTX      *ctx)
{
    ...
    cmdline_opt opts[] = {
        ...
        { ARG_OPT, ":p",         "i=N",        "Listen to TCP port N for connections (default is "TFS_DEFAULT_PORT_STR")" },
...
PSP_STARTUP_EXAMPLE(initTFS_Tutorial)
