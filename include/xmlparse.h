...
#ifndef XMLPARSEAPI
#define XMLPARSEAPI EXTERNAL_FCN
#endif
...
typedef void (*XML_StartElementHandler)(void *userData,
        const XML_Char *name,
        const XML_Char * const*atts);

typedef int32_t (*XML_EndElementHandler)(void *userData,
        const XML_Char *name);
...
typedef int32_t (*XML_NotStandaloneHandler)(void *userData);
...
typedef int32_t (*XML_ExternalEntityRefHandler)(XML_Parser parser,
...
/* This structure is filled in by the XML_UnknownEncodingHandler
   ...
   s will point32_t to a n-byte sequence where map[(uint8_t)*s] == -n.
...
typedef struct {
    int32_t map[256];
    void *data;
    uint32_t (*convert)(void *data, const char *s);
...
typedef int32_t (*XML_UnknownEncodingHandler)(void *encodingHandlerData,
        const XML_Char *name,
        XML_Encoding *info);
...
int32_t XMLPARSEAPI XML_GetSpecifiedAttributeCount(XML_Parser parser);

/* Returns the index of the ID attribute passed in the last call to
   XML_StartElementHandler, or -1 if there is no ID attribute.  Each
   attribute/value pair counts as 2; thus this correspondds to an index
   into the atts array passed to the XML_StartElementHandler. */
int32_t XMLPARSEAPI XML_GetIdAttributeIndex(XML_Parser parser);
#endif

/* Parses some input. Returns 0 if a fatal error is detected.
   The last call to XML_Parse must have isFinal true;
   len may be zero for this call (or any other). */
int32_t XMLPARSEAPI XML_Parse(XML_Parser parser, const char *s, size_t len, int32_t isFinal);
...
uint32_t XMLPARSEAPI XML_GetCurrentLineNumber(XML_Parser parser);
#if defined(NOT_USED)
uint32_t XMLPARSEAPI XML_GetCurrentColumnNumber(XML_Parser parser);
uint32_t XMLPARSEAPI XML_GetCurrentByteIndex(XML_Parser parser);

/* Return the number of bytes in the current event.
   Returns 0 if the event is in an internal entity. */

uint32_t XMLPARSEAPI XML_GetCurrentByteCount(XML_Parser parser);
#endif
...
const XML_LChar *XMLPARSEAPI XML_ErrorString(int32_t code);
